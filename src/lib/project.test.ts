...
it("inherits the project Look only when the scene has no explicit Look", () => {
  const config = createProjectConfig({ name: "Looks", prompt: "A forest", aspectRatio: "16:9", resolution: "768p", targetDurationSeconds: 0, defaultLook: "watercolor" });
  const restored = parseProjectConfig(JSON.parse(JSON.stringify(config)));
  const job = restored.generationJobs[0];
  expect(compileGenerationJobPrompt(job, [], restored.settings.defaultLook)).toContain("Watercolor");
  expect(sceneShots(job)[0].settings?.visualStyle).toBeUndefined();
  const explicit = { ...job, shots: sceneShots(job).map((shot) => ({ ...shot, settings: { visualStyle: ["claymation"] } })) };
  expect(compileGenerationJobPrompt(explicit, [], "watercolor")).toContain("Claymation");
  expect(compileGenerationJobPrompt(explicit, [], "watercolor")).not.toContain("Watercolor");
  expect(compileGenerationJobPrompt(job, [], null)).toBe(compileGenerationJobPrompt(job));
});
