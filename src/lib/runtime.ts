...
import {
  ...
  type AttentionMode, type EndpointProviderId, type EndpointProviderSettings, type EnginePathField, type GeneratorTemplate,
} from "./settings";
...
export async function getEngineStatus(settings = loadEngineSettings(), attention?: AttentionMode, loras?: TemplateLora[], mode?: GeneratorTemplate["mode"], additionalSafetensors?: GeneratorTemplate["additionalSafetensors"], motionCache?: boolean): Promise<SlopfabStatus> {
  if (!isTauri()) return DEMO_STATUS.slopfab;
  let slopfab;
  let adapterError: string | undefined;
  try {
    slopfab = engineProviderSetting(settings, undefined, attention, loras, mode, additionalSafetensors, motionCache);
  } catch (reason) {
    ...
    slopfab = engineProviderSetting(settings, undefined, attention, [], mode, additionalSafetensors, motionCache);
  }
