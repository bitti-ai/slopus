...
it("downloads TaoMate and persists manual adapters, activation, strength and order per template", async () => {
  ...
  expect(screen.getByLabelText("LoRA multiplier")).toHaveValue(1);
  fireEvent.change(screen.getByLabelText("LoRA multiplier"), { target: { value: "" } });
  expect(screen.getByRole("button", { name: "Add Lora" })).toBeDisabled();
  fireEvent.change(screen.getByLabelText("LoRA multiplier"), { target: { value: "0.75" } });
  ...
  expect(screen.getByLabelText("LoRA multiplier")).toHaveValue(0.75);
  fireEvent.click(screen.getByRole("button", { name: "Generators" }));
  fireEvent.click(screen.getByRole("button", { name: "Edit Default generator" }));
  ...
  expect(screen.getByLabelText("My style effective strength")).toHaveTextContent("-0.375");
