...
pub(super) enum RequestPurpose { Plan, Generate }
use super::*;
...
pub fn resolve_plan(
    ...
    configure_request(&api, &handle, request, &configuration, platform, RequestPurpose::Plan, references)?;
...
pub(super) fn validate_generation_controls(request: &GenerationRequest) -> Result<(), String> {
    ...
    if request.reference_paths.len() > limits.max_image_references || request.reference_video_ids.len() > limits.max_video_references || request.reference_count() > limits.max_references {
...
pub(super) fn scene_frame_window(request: &GenerationRequest, total: i32) -> Result<(i32, i32), String> {
    ...
    let limits = crate::generation::models::h3::MODEL.capabilities;
    let frames = request.frames.checked_add(limits.frame_stride - 1).ok_or("Continuation frame count overflow.")? / limits.frame_stride * limits.frame_stride;
    let offset = total.checked_sub(frames).filter(|offset| *offset >= limits.continuation_overlap)
        .ok_or("Slopfab continuation output is missing its source frames.".to_string())?;
    Ok((offset, frames))
}

pub(super) fn scene_audio_offset(frame_offset: i32, fps: f64, channels: i32, sample_rate: i32, total: usize) -> Result<usize, String> {
