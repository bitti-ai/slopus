...
pub fn create_reference_video(&self, duration: f64, settings: &BTreeMap<String, ProviderSetting>) -> Result<String, String> {
    ...
    let mut videos = self.videos.lock().map_err(|_| "Reference video lock failed.")?;
...
pub fn append_reference_video(&self, id: &str, bytes: &[u8], width: i32, height: i32, timestamp: f64) -> Result<(), String> {
    self.videos.lock().map_err(|_| "Reference video lock failed.")?
        .get_mut(id).ok_or("The prepared reference video no longer exists.")?
        .append(bytes, width, height, timestamp)
}

pub fn set_reference_video_audio(&self, id: &str, bytes: &[u8], channels: i32, sample_rate: i32) -> Result<(), String> {
