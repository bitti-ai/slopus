...
pub(super) fn configure_request(
    ...
    if configuration.animate {
        let videos = references.videos.lock().map_err(|_| "Reference video lock failed.")?;
    ...
    if let Some(path) = &request.continuation_path {
        api.set_continuation_file(handle, path, crate::generation::models::h3::MODEL.capabilities.continuation_overlap)?;
    }
