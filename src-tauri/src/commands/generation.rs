...
#[tauri::command]
pub(crate) fn create_reference_video(
...
#[tauri::command]
pub(crate) fn append_reference_video(
    state: tauri::State<'_, slopfab::SlopfabRuntime>,
    ...
    let width = reference_video_header(&request, "x-reference-width")?
        .parse()
        .map_err(|_| "Invalid frame width.")?;
    let height = reference_video_header(&request, "x-reference-height")?
        .parse()
        .map_err(|_| "Invalid frame height.")?;
    let timestamp = reference_video_header(&request, "x-reference-time")?
        .parse()
...
#[tauri::command]
pub(crate) fn set_reference_video_audio(
    state: tauri::State<'_, slopfab::SlopfabRuntime>,
    request: tauri::ipc::Request<'_>,
) -> Result<(), String> {
    let tauri::ipc::InvokeBody::Raw(bytes) = request.body() else {
    ...
    let id = reference_video_header(&request, "x-reference-id")?;
    let channels = reference_video_header(&request, "x-reference-channels")?
        .parse()
        .map_err(|_| "Invalid audio channels.")?;
    let rate = reference_video_header(&request, "x-reference-rate")?
        .parse()
...
#[tauri::command]
pub(crate) fn release_reference_videos(
    state: tauri::State<'_, slopfab::SlopfabRuntime>,
    ids: Vec<String>,
) -> Result<(), String> {
...
pub(crate) fn resolve_slopfab_plan(
    ...
    let result = validate_and_normalize_config(config).and_then(|config| {
        slopfab::resolve_plan(&request, &config.provider_settings, &state.references)
    });
    match &result {
