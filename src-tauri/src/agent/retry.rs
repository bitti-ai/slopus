...
impl RetryBudget {
    pub(super) fn correction(&mut self, issue: &crate::project::validation::issue::ValidationIssue) -> Result<usize, String> {
        if self.previous.as_ref() == Some(&issue.key) { self.round += 1; }
        else { self.previous = Some(issue.key.clone()); self.round = 1; }
...
mod tests {
    ...
    fn changed_wording_does_not_reset_the_same_issue_budget() {
        ...
        for round in 1..=3 {
            let issue = ValidationIssue::new("scene.invalid", Some("scene-1".into()), "duration", format!("Bad value {round}"));
            assert_eq!(budget.correction(&issue).unwrap(), round);
        }
        assert!(budget.correction(&ValidationIssue::new("scene.invalid", Some("scene-1".into()), "duration", "Different wording".into())).is_err());
        assert_eq!(budget.correction(&ValidationIssue::new("scene.invalid", Some("scene-1".into()), "references", "Missing reference".into())).unwrap(), 1);
    }
}
pub(super) fn validation_retry_prompt(original: &str, previous: &str, failure: &str, round: usize) -> String {
    format!(
