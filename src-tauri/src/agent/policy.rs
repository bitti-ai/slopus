...
pub(crate) fn validate_agent_scene_conventions(
    ...
    validate_scene_policy(before, next).map_err(|issue| issue.message)
}

pub(super) fn validate_scene_policy(
    before: &ProjectConfig,
    next: &ProjectConfig,
        ...
        .filter(|reference| new_reference_ids.contains(reference.id.as_str()))
    {
        if reference.kind != "text" {
            return Err(ValidationIssue::new("reference.kind", Some(reference.id.clone()), "kind", format!(
        ...
        if definition.trim().is_empty() {
            return Err(ValidationIssue::new("reference.description", Some(reference.id.clone()), "description", format!(
                "Agent-created reference '{}' needs a complete visual description before it is used in shots.",
                reference.id
            )));
        }
        if reference.intended_use.is_empty() {
            return Err(ValidationIssue::new("reference.usage", Some(reference.id.clone()), "intendedUse", format!(
    ...
    for job in &next.generation_jobs {
        ...
        let duration = job.duration_seconds.unwrap_or_else(|| {
            model
        ...
        for shot in shots {
            ...
            if let Some(model) = model {
                (model.validate_shot_settings)(before_shot, shot, earliest_shot_id).map_err(
                    |message| {
                        ValidationIssue::new(
                            "shot.settings",
                            Some(shot.id.clone()),
            ...
            if action_contains_dialogue(&shot.action) {
                return Err(ValidationIssue::new("shot.dialogue", Some(shot.id.clone()), "action", format!(
            ...
            {
                return Err(ValidationIssue::new("shot.language", Some(shot.id.clone()), "speechLanguage", format!(
                    "Shot '{}' in scene '{}' has speech but no speechLanguage. Keep the spoken line in shot.speech and set its language.",
                    shot.id, job.id
                )));
            }
            for reference_id in action_reference_ids(&shot.action).map_err(|message| {
                ValidationIssue::new("shot.citation", Some(shot.id.clone()), "action", message)
            })? {
                let reference = next
                    .references
                    .iter()
                    ...
                    .ok_or_else(|| {
                        ValidationIssue::new("shot.unknown_reference", Some(shot.id.clone()), "action", format!(
                ...
                if !job.reference_ids.iter().any(|id| id == reference_id) {
                    return Err(ValidationIssue::new("shot.binding", Some(shot.id.clone()), "referenceIds", format!(
    ...
    if changed_scene {
        for reference in next.references.iter().filter(|reference| {
            ...
            if !cited_new_references.contains(reference.id.as_str()) {
                return Err(ValidationIssue::new("reference.uncited", Some(reference.id.clone()), "action", format!(
